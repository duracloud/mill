...
public class LogManagerImplTest extends AbstractTestBase {
    ...
    /**
     * Test method for {@link org.duracloud.mill.audit.generator.LogManagerImpl#write(org.duracloud.mill.db.model.JpaAuditLogItem)}.
     * @throws IOException 
     */
    @Test
    public void testWrite() throws IOException {
        manager = new LogManagerImpl(endpoint, logsRootDir.getAbsolutePath(), repo){
            /* (non-Javadoc)
             * @see org.duracloud.mill.audit.generator.LogManagerImpl#createSpaceLog(org.duracloud.mill.audit.generator.LogKey)
             */
            @Override
...
//        expect(item.getContentMd5()).andReturn(checksum).atLeastOnce();
    ...
    public void testFlushLogs() throws IOException{
        ...
        expect(this.endpoint.syncFileAndReturnDetailedResult(isA(MonitoredFile.class),
                                                             eq(logsRootDir)))
                .andReturn(SyncResultType.ADDED).times(fileCount);
